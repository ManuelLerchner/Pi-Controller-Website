...
class HandleIO {
    ...
    handleEvents() {
        this.io.on("connection", (socket) => {
            ...
            var UpdateInterval = setInterval(
                function () { this.updatePCState(process.env.PC_IP); }.bind(this),
            ...
            socket.on("relay authentification", async (data) => {
                ...
                Auth.login(data).then(([success, response]) => {
                    var htmlResponse = `<span style='color: ${success ? "green" : "red"};'>${response}</span>`;
    ...
    async handleSessions(socket) {
        return await Session.checkIfTooManyRequests(this.getIp(socket));
    }
    ...
    togglePCState() {
        if (pcState == true) { PC.off(); } else { PC.on(); }
    }
